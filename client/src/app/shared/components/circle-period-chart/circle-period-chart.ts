...
export class CirclePeriodChart {
  ...
  @Input() ovulationDay: number = 14;
  @Input() startPeriodDay: number = new Date().getDate();
  segments: { start: number; len: number; color: string }[] = [];
  startDate: any;
  endDate: any;
  cycleLength = new Date(
  ...
  private updatePositions() {
    // today dot
    const todayAngle =
      (this.today / this.cycleLength) * 2 * Math.PI - Math.PI / 2;
    this.todayX = 132 * Math.cos(todayAngle);
    this.todayY = 132 * Math.sin(todayAngle);

    // ovulation heart
    const ovAngle =
      (this.ovulationDay / this.cycleLength) * 2 * Math.PI - Math.PI / 2;
  ...
  updateSegmentsFromDate(startDate: any, endDate: any) {
    ...
    // Day index in cycle (0 = start of cycle)
    const cycleStartIndex = 0;

    // Convert start date -> cycle day index (0-based)
    const SelectedStartDate = start.getDate(); // e.g. 5th of month

    if (cycleStartIndex + periodLength <= this.cycleLength) {
      this.segments = [
        { start: cycleStartIndex, len: periodLength, color: '#A01D1E' }, // period
        // { start: 11, len: 6, color: '#4DE2EF' }, // fertile window
        // { start: 14, len: 0.4, color: '#0FA3B1' }, // ovulation marker
        // { start: 22, len: 5, color: '#463BAC' }, // PMS
      ];
    } else {
      // Wrap around the end of the cycle
      const firstPart = this.cycleLength - cycleStartIndex;
      const secondPart = periodLength - firstPart;

      this.segments = [
        { start: cycleStartIndex, len: firstPart, color: '#A01D1E' },
        // { start: 0, len: secondPart, color: '#A01D1E' },
        // { start: 11, len: 6, color: '#4DE2EF' },
        // { start: 14, len: 0.4, color: '#0FA3B1' },
        // { start: 22, len: 5, color: '#463BAC' },
      ];
    }
