...
import { CirclePeriodChart } from '@app/shared/ui/circle-period-chart/circle-period-chart';
import { PregnancyHeroSvgComponent } from '@app/shared/ui/pregnancy-hero-svg/pregnancy-hero-svg.component';
import { MenopauseHeroSvgComponent } from '@app/shared/ui/menopause-hero-svg/menopause-hero-svg.component';
import {
  buildMenopauseHomeContent,
  translateMenopauseText,
  type MenopauseGuideCard,
  type MenopauseHomeContent,
} from '@app/shared/utils/menopause-home-content.util';
import { DailyInsightsStoryModalComponent } from '@app/shared/ui/daily-insights-story-modal/daily-insights-story-modal.component';
import { FertilityOverviewSheetComponent } from '@app/shared/ui/fertility-overview-sheet/fertility-overview-sheet.component';
import {
  FertilityResults,
  FertilityResultsModalComponent,
} from '@app/shared/ui/fertility-results-modal/fertility-results-modal.component';
import {
  PregnancyResults,
  PregnancyResultsModalComponent,
} from '@app/shared/ui/pregnancy-results-modal/pregnancy-results-modal.component';
import { PregnancySetupSheetComponent } from '@app/shared/ui/pregnancy-setup-sheet/pregnancy-setup-sheet.component';
import { getAllSymptoms } from '@app/shared/constants/symptoms-config';
import type { UserInfo } from '@app/shared/interfaces/user-info-api.interface';
import type { DailyInsightTopic } from '@app/shared/models/daily-insight-topic.model';
import { SymptomsDto } from '@app/shared/models/symptoms.dto';
import {
  pregnancyWeekIllustrationAlt,
  pregnancyWeekIllustrationUrl,
} from '@app/shared/pregnancy-week-illustration';
import {
  BabyDevelopmentService,
  BabySizeData,
} from '@app/shared/services/baby-development.service';
...
import { PeriodCycleStateService } from '@app/shared/services/period-cycle-state.service';
...
} from '@app/features/home/services/home-facade.service';
import { SHARED_STANDALONE_IMPORTS } from '@app/shared/shared-standalone';
import { pregnancyDashboardInsightFromWeek } from '@app/shared/utils/pregnancy-dashboard-insight.util';
import {
  cycleDayFromLmpIso,
  parseCycleViewDateKey,
} from '@app/shared/utils/cycle-day.util';
import {
...
} from '@app/features/home/data/home-baby-week-copy';
import { HOME_POSTPARTUM_WEEK_SAMPLES } from '@app/features/home/data/home-postpartum-sample.data';
...
export class HomeComponent implements OnInit, OnDestroy, ViewWillEnter {
  ...
  private applyJourneyStateToView(state: HomePageJourneyState) {
    ...
    this.lastPeriodStartIso = storedLmp
      ...
      : null;
    // Local period logs (picker / calendar) must beat stale dashboard-inferred starts.
    if (storedLmp) {
      const day = storedLmp.includes('T')
        ? storedLmp.split('T')[0]
        : storedLmp.slice(0, 10);
      this.periodStartDate = new Date(`${day}T12:00:00`);
    } else {
  ...
  /** Hero advice preview length — full AI period-day copy opens via Continue. */
  private readonly cycleTodayAdvicePreviewMax = 110;

  getCycleTodayAdvicePreview(): string {
    return this.truncateInsightTeaser(
      this.getCycleTodayAdvice(),
      this.cycleTodayAdvicePreviewMax,
    );
  }

  isCycleTodayAdviceLong(): boolean {
    return (
      this.getCycleTodayAdvice().trim().length > this.cycleTodayAdvicePreviewMax
    );
  }

  async openCycleTodayAdviceStory(): Promise<void> {
    const full = this.getCycleTodayAdvice();
    const phase = this.getCyclePhaseTone();
    await this.openDailyInsightStory({
      id: 'cycle-today-advice',
      categoryLabel: this.tr(`home.cyclePhase.${phase}`),
      teaser: this.truncateInsightTeaser(full),
      accentHex: phase === 'period' ? '#f43f5e' : '#db2777',
      ionIcon: this.getCyclePhaseStripIcon(),
      slides: [
        {
          title: this.getCyclePhaseGuideHeadline(),
          body: full,
        },
      ],
    });
...
Generated by DoreHealth App To Elahi Fatat besham Azizam`;
  ...
  getCyclePhaseGuideHeadline(): string {
    // Client i18n — dashboard phase-guide headline is English-only today.
  ...
  getCycleTodayAdvice(): string {
    ...
    // Hero advice uses client i18n — dashboard insight is English-only today.
    return this.buildLocalizedCycleTodayAdvice(tone);
  }

  /** Localized science-informed fallback while dashboard insight is loading. */
  private buildLocalizedCycleTodayAdvice(
    tone: 'period' | 'follicular' | 'fertile' | 'luteal',
  ): string {
    const plen = Math.max(1, this.periodLength || 5);
    ...
    if (tone === 'period') {
      ...
      if (day === 3) {
        return this.tr('home.cycleToday.period.day3');
      }
      if (day >= plen) {
  ...
  updateCycleDay() {
    const lmpIso = this.cycleSettings.lastPeriodStartDate();
    if (!lmpIso) {
      this.periodStartDate = null;
      this.lastPeriodStartIso = null;
      this.currentCycleDay = 0;
    ...
    const day = lmpIso.includes('T') ? lmpIso.split('T')[0] : lmpIso.slice(0, 10);
    this.lastPeriodStartIso = day;
    this.periodStartDate = new Date(`${day}T12:00:00`);
    ...
    if (selectedKey) {
      const parsed = parseCycleViewDateKey(selectedKey);
      if (parsed) {
        viewDate = parsed;
      }
    }

    this.currentCycleDay = cycleDayFromLmpIso(
      day,
      viewDate,
      this.currentCycleLength || 28,
    );
  }
  ...
  pregSlideAriaLabel(slide: PregnancyFeatureSlide): string {
    ...
    const footnote = slide.footnoteKey
      ? slide.footnoteParams
        ? this.tr(slide.footnoteKey, slide.footnoteParams)
        : this.tr(slide.footnoteKey)
      : '';
    const learnMore = this.tr('home.learnMore');
    return [eyebrow, slide.headline, slide.body, footnote, learnMore]
      .filter(Boolean)
      .join('. ');
  }
